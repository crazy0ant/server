...
class Room extends EventEmitter
{
	...
	constructor({ roomId, protooRoom, mediasoupRouter, audioLevelObserver, bot, drawingBoard, otherState })
	{
		super();
	...
	async _handleProtooRequest(peer, request, accept, reject)
	{
		switch (request.method)
		{
			...
			case 'join':
			{
				...
				peer.data.isAVClient = isAVClient;
				...
				const peerInfos = joinedPeers
					...
					.map((joinedPeer) => ({
						...
						displayName : joinedPeer.data.displayName,
						device      : joinedPeer.data.device,
						isAVClient  : joinedPeer.data.isAVClient
					}));

				accept({ peers: peerInfos, otherState: this._otherState });
				...
				for (const otherPeer of this._getJoinedPeers({ excludePeer: peer }))
				{
					otherPeer.notify(
						'newPeer',
						{
							...
							isAVClient  : peer.data.isAVClient,
			...
			case 'joinRoom':
			{
				...
				accept({ peers: peerInfos, otherState: this._otherState,devices:this._broadcastWebcams });
			...
			case 'setOtherState':
			{
				...
				if(micState==='on' || micState ==='off'){
					this._otherState.micState = micState;
				}
				if(webcamState==='on' || webcamState==='off'){
					this._otherState.webcamState = webcamState;
				}
				//广播给房间其他人
				...
				for (const otherPeer of this._getJoinedPeers({ excludePeer: peer }))
				{
					otherPeer.notify(
						'setOtherState',
						{
							id          : peer.id,
							displayName : peer.data.displayName,
							otherState  : {micState,webcamState}
						})
						.catch(() => {});
				}
				accept({
			...
			case 'setBroadcastWebcam'://监控摄像头广播
			{
				const {type,webcamId,webcam} = request.data;
				if(type=='add'){
					this._broadcastWebcams[webcamId] = webcam;
				}else if(type=='remove'){
					delete this._broadcastWebcams[webcamId];
				}
					for (const otherPeer of this._getJoinedPeers({ excludePeer: peer }))
					{
						otherPeer.notify(
							'setBroadcastWebcam',
							{type,webcamId,webcam}
							).catch(() => {});
					}
				accept();
				break;
			...
			case 'applySpeech'://申请发言
			{
				//转发peerId，displayName，
				for (const otherPeer of this._getJoinedPeers({ excludePeer: peer }))
				{
					otherPeer.notify(
						'applySpeech',
			...
			case 'applySpeechRes'://申请发言
			{
				const {peerId, result, displayName} = request.data;
				//转发peerId，displayName，
				for (const otherPeer of this._getJoinedPeers({ excludePeer: peer }))
				{
					otherPeer.notify(
						'applySpeechRes',
						{peerId, result, displayName})
						.catch(() => {});
				}
				accept();
				break;
			}
			case 'setOtherMediaState'://申请发言
			{
				const {peerId, type, state, displayName} = request.data;
				//转发peerId，displayName，
				for (const otherPeer of this._getJoinedPeers({ excludePeer: peer }))
				{
					otherPeer.notify(
						'setOtherMediaState',
						{peerId, type, state, displayName}
						).catch(() => {});
				}
				accept();
				break;
			...
			case 'drawingBoardRelay':
			{
				...
				//存储白板数据到服务器
				switch (type) {
					case 'drawLine'://客户端画线操作
					{
						const {pageNum, lineObj} = data;
						this._drawingBoard.addLine(pageNum,lineObj);
						break;
					}
					case 'toPage'://客户端翻页
					{
						const {pageNum} = data;
						this._drawingBoard.toPage(pageNum);
						break;
					}
					case 'addPage'://客户端添加新一页
					{
						this._drawingBoard.addPage();
						break;
					}
					case 'cleanPage'://客户端清空当前页面
					{
						const {pageNum} = data;
						this._drawingBoard.cleanPage(pageNum);
						break;
					}
					case 'isShow'://客户点白板打开状态
					{
						const {isShow} = data;
						this._drawingBoard.setShow(isShow);
						break;
					}
				}
				// Notify other joined Peers.
				for (const otherPeer of this._getJoinedPeers({ excludePeer: peer }))
				{
					otherPeer.notify( 'drawingBoardRelay', { type,data, })
						.catch(() => {console.log('drawingBoardRelay发生错误')});
				}
				accept();
				break;
			}
			case 'drawingBoardSync'://同步白板数据
			{
				peer.notify( 'drawingBoardSync', this._drawingBoard)
					.catch(() => {console.log('drawingBoardSync发生错误')});
				accept();
				break;
			}
			default:
